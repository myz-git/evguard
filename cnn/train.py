...
def main():
    ...
    # 先划分索引，再构建不同 transform 的 train/val 数据集
    all_images = positive_paths + negative_paths
    ...
    train_dataset = IconDataset(train_images, train_labels, transform=train_transform)
    val_dataset = IconDataset(val_images, val_labels, transform=val_transform)
    ...
    model, best_acc = train_model(model, train_loader, val_loader, num_epochs=50, device=device)
    ...
    logging.info(f"模型已保存到: {model_path} (best val acc: {best_acc}%)")

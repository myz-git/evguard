...
# 与 train.py 一致：模型在 cnn/model_cnn，数据在仓库根目录 traindata/、icon/
...
def verify_icon(icon_name, max_attempts=3, template_threshold=0.86, model_threshold=0.9):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = IconCNN().to(device)
    model_path = os.path.join(_SCRIPT_DIR, 'model_cnn', f"{icon_name}_classifier.pth")
    ...
    template_path = os.path.normpath(os.path.join(_SCRIPT_DIR, '..', 'icon', f"{icon_name}-1.png"))
...
def evaluate_accuracy(icon_name):
    ...
    traindata_root = os.path.normpath(os.path.join(_SCRIPT_DIR, '..', 'traindata'))
    pos_dir = os.path.join(traindata_root, f'{icon_name}-1')
    neg_dir = os.path.join(traindata_root, f'{icon_name}-0')

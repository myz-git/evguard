...
class EvGuardApp:
    ...
    def _build_process_panel(self, parent):
        ...
        for name in PROCESS_ORDER:
            ...
            start_btn = tk.Button(btns, text=">> START", width=10, command=lambda n=name: self.start(n))
            self._style_button(start_btn, "primary")
            start_btn.pack(side="left", padx=(0, 6))
            stop_btn = tk.Button(btns, text="|| STOP", width=10, command=lambda n=name: self.stop(n))
    ...
    def _build_log_panel(self, parent):
        ...
        tk.Checkbutton(tool, text="自动滚动", variable=self.auto_scroll_var, bg=COLORS["surface_alt"], fg=COLORS["muted"]).pack(side="right")
        clear_btn = tk.Button(tool, text="清空", command=self.clear_logs)
    ...
    def _on_button_press(self, button):
        if str(button.cget("state")) != "disabled":
            button.configure(relief="sunken", padx=12, pady=5)

    def _on_button_release(self, button):
        if str(button.cget("state")) != "disabled":
            button.configure(relief="raised", padx=13, pady=6)

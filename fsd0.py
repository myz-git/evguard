...
from utils import play_sound_wav, log_message, print_startup, safe_find_icon, safe_find_any_icon, hscrollscreen, rolljump, screen_regions, find_txt_ocr, dump_ocr_texts, speak, suppress_not_found_warnings_console, is_game_exefile_running, log_screen_region_layout, is_destination_reached, ROLLJUMP_ARRIVED, get_jump_icon_template_threshold


ensure_license_or_exit()

# fsd0 执行时不向控制台输出「未找到」类 WARNING（仍写 evguard.log）
...
def main():
    ...
    if not is_game_exefile_running():
        ...
        return 1
    print("游戏检测中...请稍后...")
    log_screen_region_layout("FSD0区域布局", ["mid_left_panel", "control_panel", "full_right_panel"])
    # 只在循环第一次执行时播放一次启动音效
        ...
        if state == "set_destination":
            ...
            if safe_find_any_icon(["jump3", "jump3s"], region_full_right, max_attempts=1, threshold=jump_threshold):
                ...
                if result is True:
                    ...
                    time.sleep(20)
                    state = "check_dock"
                elif result == ROLLJUMP_ARRIVED or is_destination_reached(mid_left_panel):
            ...
            while running:
                ...
                if find_txt_ocr("离站", max_attempts=1, region=region_full_right, log_miss=False):
